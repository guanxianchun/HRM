package com.sailing.hrm.service.report;
...
public class BirtEngine {
	...
	public static synchronized IReportEngine getBirtEngine() {
		if (birtEngine == null) {
			...
			try {
				String en_path = BirtEngine.class.getClassLoader().getResource("").getPath();
				en_path = en_path.replace("/classes/", "/ReportEngine");

				config.setEngineHome(en_path);
				config.setLogConfig(System.getProperty("user.dir") + "/logs/birt_log", Level.FINE);
