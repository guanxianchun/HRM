package com.sailing.hrm.service.menu;
...
import com.sailing.hrm.persistent.common.Page;
import com.sailing.hrm.persistent.common.PropertyFilter;
import com.sailing.hrm.persistent.menu.bean.Menu;
import com.sailing.hrm.persistent.menu.dao.MenuDao;
import com.sailing.hrm.service.resources.ResourceManager;
...
public class MenuManager {
	...
	public List<Menu> getAllowedAccessMenu(Long userId) {
		...
		sqlBuffer.append(" select m.id,m.name,m.pid,m.description,m.orderby," +
				"m.action,m.url,m.data,m.title, m.icon, m.iconopen,m.opened, m.src,m.open_icon,m.target  from sec_menu m ");
		...
		sqlBuffer.append(" select m.id,m.name,m.pid,re.source as description,m.orderby," +
				"m.action,m.url,m.data,m.title, m.icon, m.iconopen,m.opened, m.src,m.open_icon ,m.target  from sec_resource re ");
		...
		sqlBuffer.append(" select m.id,m.name,m.pid,re.source as description,m.orderby," +
				" m.action,m.url,m.data,m.title, m.icon, m.iconopen,m.opened, m.src,m.open_icon ,m.target from sec_user u ");
