package com.sailing.hrm.persistent.report;
...
public class SQLTool {
	...
	public String appendSql(String sql, String ifName, String way, String ifValue) {
		sql = sql.trim();
		int j = sql.length() - 1;
		int i = 1;
		int where = 0;
		...
		if (i == -1 && where == 1) {// 不存在and,不存在where 新添加
			if (way.equals("equal")) {
				sql = sql + " " + ifName + "= ?";
				parm.clear();
				parm.add(ifValue);
	...
	// 大于2个以上的关键字删除
	@SuppressWarnings("rawtypes")
	public int deleteRecords(String table, ArrayList IDlist, ArrayList inputValue) {
	 ...
	 * @version 0.1
	 */
	public static String getPageMySQL(String sql, String orderKey, String upORDown, String start,
			String pageSize) {
		if (sql != null && !"".equals(upORDown) && upORDown != null)
			sql = sql + " ORDER BY " + orderKey + " " + upORDown + "  limit " + start + " , "
					+ pageSize;
		else
			sql = sql + " ORDER BY " + orderKey + " desc limit " + start + " , " + pageSize;
		return sql;
	 ...
	 * @version 0.1
	 */
	public static String getPageByOracle(String sql, String start, String pageSize) {
		String tmp = sql;
		sql = sql.replaceFirst("select", "");
		if (!sql.contains("where"))
			// select * from (select rownum no,* from userlist where rownum<=m)
			// where no>=n;
			sql = "select * from ( select  rownum no," + tmp + " where rownum<=" + pageSize
					+ ") where no >=" + start;
		else
			sql = "select * from ( select  rownum no," + tmp + " and rownum<=" + pageSize
					+ ") where no >=" + start;
		return sql;
	 ...
	 * @version 0.1
	 */
	public static String getRecuSQl(String tab, String parentKey, String childKey, Integer val,
			boolean include_self) {
		String recuSQl = null;
		if (tab != null && parentKey != null && childKey != null && val != null) {
			if (include_self) {
				recuSQl = "select t2." + childKey + ",  t2." + parentKey + " from " + tab + " t1, "
						+ tab + " t2  " + " where t1." + childKey + "=t2." + parentKey
						+ " and (t1." + childKey + "=" + val + " or t1." + parentKey + "=" + val
						+ ") union " + " select t3." + childKey + ",  t3." + parentKey + " "
						+ " from " + tab + " t3 where t3." + childKey + "=" + val;
			} else {
				recuSQl = "select t2." + childKey + ", t2.name, t2." + parentKey + " from " + tab
						+ " t1, " + tab + " t2  " + " where t1." + childKey + "=t2." + parentKey
						+ "  and (t1." + childKey + "=" + val + " or t1." + parentKey + "=" + val
						+ ")";
			}
		}
	 ...
	 * @version 0.1
	 */
	public static String appendUpdateSql(String before_sql, String updateKeyName, String keyType,
			String keyValue) {
		String up_sql = "";
		if (updateKeyName != null && !"".equals(keyValue)) {
			if ("String".equals(keyType))
				...
				up_sql = updateKeyName + "= str_to_date('" + keyValue + "', '%Y-%m-%d')";
			else
				up_sql = updateKeyName + "=" + keyValue;
		}
