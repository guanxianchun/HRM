...
public class MDBTool implements IMDBTool {
	 ...
	 * @version 0.1
	 */
	private void setParamValue(PreparedStatement theStmt, List<?> params, int type[])
			throws Exception {
	 ...
	 * @version 0.1
	 */
	private void setProcedureParamValue(PreparedStatement theStmt, List<?> params, int beforecall)
			throws Exception {
		if (params == null || params.size() == 0)
			return;
	...
	private List<JSONObject> getDataSetToJson(ResultSet rs) throws Exception {
		...
		while (s < i) {
			...
			for (int t = 1; t <= j; t++) {
				...
				if (rs.getObject(keyName) instanceof Timestamp
						|| rs.getObject(keyName) instanceof Date) {
	...
	public ResultSet executeQuery(String sql, List parm) throws Exception {
		...
		try {
			if (conn == null || conn.isClosed())
				conn = ds.getConnection();
			psQuery = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE,
	...
	public List<JSONObject> executeJSONQuery(String sql, List<String> parm) throws Exception {
		...
		try {
			if (conn == null || conn.isClosed())
				conn = ds.getConnection();
			psQuery = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE,
					ResultSet.CONCUR_READ_ONLY);
			setParamValue(psQuery, parm);
			resultset = psQuery.executeQuery();
	...
	public int executeUpdate(String sql, List parm) throws Exception {
		...
		try {
			if (conn == null || conn.isClosed())
				conn = ds.getConnection();
			psUpdate = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE,
	 ...
	 * @version 0.1
	 */
	public ResultSet executeQueryProcedure(String procedureName, List<?> params, boolean scrollable)
			throws Exception {
		if (conn == null || conn.isClosed())
			conn = ds.getConnection();
		logger.info("调用存储过程！");
	...
	public ArrayList<?> executeProcedure(String procedureName, List<String> inparams,
			List<String> outparams, List<String> returnparams) throws Exception {
		...
		try {
			...
			for (int j = 0; j < outparams.size(); j++) {
				...
				if (value instanceof Integer) {
					...
					returnOutparams
							.add(callprocedure.getString(inparamsNums + 1 + j - returnParam));
					continue;
				} else {
